...
void InputLayoutD3D11::AddInputElement(const char* semanticName, DXGI_FORMAT format, size_t inputSlot)
{
	...
	{ semanticName, 0, format, 0, (UINT)inputSlot, D3D11_INPUT_PER_VERTEX_DATA, 0 };
...
{
	if (FAILED(device->CreateInputLayout(this->elements.data(), (UINT)this->elements.size(), vsDataPtr, vsDataSize, this->inputLayout.GetAddressOf())))
	{
...
ID3D11InputLayout* InputLayoutD3D11::GetInputLayout() const { return this->inputLayout.Get(); }
